...
function SlotMachine() {
  ...
  const handleWinnings = (winningCombos) => {
    if (winningCombos.length > 0) {
      ...
      // Apply multiplier to gems if free spins are active
      const finalGems = activeMultiplier ? baseGems * activeMultiplier : baseGems;
  ...
  function handleSpinClick() {
    ...
    if (freeSpins > 0) {
      setFreeSpins(prev => {
        const newValue = Math.max(0, prev - 1); // Ensure it never goes below 0
        if (newValue === 0) {
          setActiveMultiplier(0); // Reset multiplier when free spins are depleted
        }
        return newValue;
      });
    } else {
    ...
    <main className="min-h-screen bg-gradient-to-b from-purple-900 via-purple-800 to-pink-800">
      ...
      <SpinningWheel 
        ...
        onComplete={(spins, multiplier) => {
          ...
          setActiveMultiplier(prev => prev + multiplier); // Add new multiplier to existing one
        }}
      />
    </main>
  );
