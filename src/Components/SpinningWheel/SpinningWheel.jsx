...
function SpinningWheel({ isVisible, onComplete }) {
  ...
  useEffect(() => {
    if (wheelState.readyToClose) {
      const closeTimer = setTimeout(() => {
        resetWheel();
        onComplete(wheelState.result * wheelState.slotNumber);
      }, 2000); // Reduced delay to 2000ms to avoid overlapping with slot spin
  ...
  const startSpin = () => {
    ...
    setTimeout(() => {
      ...
      // Start the slot spin after the wheel spin completes
      setTimeout(() => {
        setWheelState((prev) => ({
          ...prev,
          isSlotSpinning: true,
        ...
        >
          ...
          <div className="relative max-w-[500px] w-full aspect-square p-8">
            <div className="relative w-full h-full">
              {/* Stationary Slot Machine Container */}
            ...
            {wheelState.result && wheelState.slotNumber && (
              ...
              >
                <div className="text-4xl font-bold text-yellow-400">
                  {wheelState.result}x × {wheelState.slotNumber} = {wheelState.result * wheelState.slotNumber}
                </div>
              </motion.div>
            )}
          </div>
